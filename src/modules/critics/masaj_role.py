...
class MASAJRoleCritic(nn.Module):
    ...
    def _build_inputs(self, batch, bs, max_t):
        inputs = [batch["obs"],
                  th.eye(self.n_roles, device=batch.device).unsqueeze(0).unsqueeze(0).expand(bs, max_t, -1, -1)]
        # state, obs, action
        ...
        inputs = th.cat([x.reshape(bs, max_t, self.n_roles, -1) for x in inputs], dim=-1)
